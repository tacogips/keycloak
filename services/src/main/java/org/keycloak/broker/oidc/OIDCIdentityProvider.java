...
public class OIDCIdentityProvider extends AbstractOAuth2IdentityProvider<OIDCIdentityProviderConfig> implements ExchangeExternalToken {
    ...
    public BrokeredIdentityContext getFederatedIdentity(String response) {
				logger.info("===== start getFederatedIdentity in OIDCProvider");
        AccessTokenResponse tokenResponse = null;
        try {
            tokenResponse = JsonSerialization.readValue(response, AccessTokenResponse.class);
        } catch (IOException e) {
        ...
        try {
						...
						logger.info("===== start extract identify in OIDCProvider");
    ...
    protected BrokeredIdentityContext extractIdentity(AccessTokenResponse tokenResponse, String accessToken, JsonWebToken idToken) throws IOException {
        ...
        if (!getConfig().isDisableUserInfoService()) {
            String userInfoUrl = getUserInfoUrl();
						logger.infof("====  extractIdentity userInfoUrl : %s ",userInfoUrl);
            if (userInfoUrl != null && !userInfoUrl.isEmpty()) {
                ...
                if (accessToken != null) {
                    ...
                    email = getJsonProperty(userInfo, "email");
										...
										logger.infof("====  fetched user info : %", userInfo.toString());
										logger.infof("====  extractIdentity in OIDC fetched user info: id: %s, name:%s,givenName:%s, familyName:%s, preferredUsername:%s, email:%s ", id,name,givenName,familyName,preferredUsername,email );
